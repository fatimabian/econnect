...
if (isset($_POST['add_admin'])) {
    ...
    if ($check->num_rows > 0) {
        ...
        $stmt = $conn->prepare("INSERT INTO barangay_admins (barangay, username, password, status) VALUES (?, ?, ?, ?)");
        $stmt->bind_param("ssss", $barangay, $username, $password, $status);
        $stmt->execute();
...
if (isset($_POST['edit_admin'])) {
    ...
    if (!empty($_POST['edit_password'])) {
        ...
        $stmt = $conn->prepare("UPDATE barangay_admins SET barangay=?, username=?, status=?, password=? WHERE id=?");
        $stmt->bind_param("ssssi", $barangay, $username, $status, $password, $id);
    } else {
        $stmt = $conn->prepare("UPDATE barangay_admins SET barangay=?, username=?, status=? WHERE id=?");
...
.modal {
    ...
    padding-top: 80px !important; /* Add padding to prevent hiding behind header */
}

.modal-dialog {
    margin-top: 60px !important; /* Push modal down */
    margin-bottom: 60px !important; /* Add bottom margin for mobile */
}
...
@media (max-width: 768px) {
    ...
    .table {
        min-width: 600px; /* Allow horizontal scrolling on mobile */
    }
...
<div class="container-fluid px-2 px-md-3 py-2 fade-in">
    ...
    <div class="page-header">
        <div class="d-flex flex-column flex-sm-row justify-content-between align-items-start align-items-sm-center">
            <h1>
                <i class="fas fa-user-shield me-2"></i> Barangay Admin Accounts
            </h1>
            <button class="btn btn-black mt-2 mt-sm-0" data-bs-toggle="modal" data-bs-target="#addAdminModal">
                <i class="fas fa-user-plus me-2"></i> Add Admin
            </button>
        </div>
        <p class="mb-0 mt-2 opacity-75">Manage barangay administrator accounts and permissions</p>
    </div>
    ...
    <div class="row mb-3">
        <div class="col-md-6 mb-2 mb-md-0">
            <div class="search-container">
                ...
                <input type="text" id="searchBar" class="form-control" placeholder="Search admin...">
            </div>
        </div>
        <div class="col-md-6">
    ...
    <div class="card">
        <div class="card-header d-flex flex-column flex-sm-row justify-content-between align-items-start align-items-sm-center">
            <h5 class="mb-0 mb-sm-0"><i class="fas fa-list me-2"></i> Barangay Administrators</h5>
            <span class="badge bg-light text-dark mt-1 mt-sm-0">
                <i class="fas fa-users me-1"></i> Total: <?= count($admins) ?>
            </span>
        </div>
                ...
                <table class="table table-hover mb-0">
                    <thead>
                        <tr>
                            ...
                            <th style="min-width: 180px;" class="text-center">Actions</th>
                        </tr>
                    </thead>
                    <tbody id="adminTable">
                        <?php if (empty($admins)): ?>
                            <tr>
                                <td colspan="5" class="text-center py-4">
                                    <div class="empty-state">
                                        ...
                                        <h4 class="mt-2">No Administrators Found</h4>
                                        <p class="mb-0">Add your first barangay administrator to get started</p>
                                        <button class="btn btn-black mt-3" data-bs-toggle="modal" data-bs-target="#addAdminModal">
                                            <i class="fas fa-user-plus me-2"></i> Add Admin
                                        </button>
                                    </div>
                                </td>
                            ...
                            <tr data-status="<?= strtolower($row['status']) ?>">
                                ...
                                <td>
                                    <div class="d-flex align-items-center">
                                        ...
                                        <span class="text-truncate" style="max-width: 150px;"><?= htmlspecialchars($row['barangay']) ?></span>
                                    </div>
                                </td>
                                <td>
                                    <div class="d-flex align-items-center">
                                        <i class="fas fa-user text-secondary me-2"></i>
                                        <span class="text-truncate" style="max-width: 120px;"><?= htmlspecialchars($row['username']) ?></span>
                                    </div>
                                </td>
                                <td>
                ...
                <div class="card-body py-2">
                    <div class="d-flex justify-content-between align-items-center">
                        <div>
                            ...
                            <h4 class="mb-0 fw-bold"><?= count($admins) ?></h4>
                        </div>
                ...
                <div class="card-body py-2">
                    <div class="d-flex justify-content-between align-items-center">
                        <div>
                            ...
                            <h4 class="mb-0 fw-bold"><?= count(array_filter($admins, fn($a) => $a['status'] === 'Active')) ?></h4>
                        </div>
                ...
                <div class="card-body py-2">
                    <div class="d-flex justify-content-between align-items-center">
                        <div>
                            ...
                            <h4 class="mb-0 fw-bold"><?= count(array_filter($admins, fn($a) => $a['status'] === 'Inactive')) ?></h4>
                        </div>
        ...
        <form method="POST" class="modal-content" id="addAdminForm">
            <div class="modal-header">
                <h5 class="modal-title">
                    <i class="fas fa-user-plus me-2"></i> Add Barangay Admin
                </h5>
            ...
            <div class="modal-footer">
                ...
                <button type="submit" name="add_admin" class="btn btn-black">
...
<div class="modal fade" id="editAdminModal" tabindex="-1" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered">
        <form method="POST" class="modal-content" id="editAdminForm">
            <div class="modal-header">
                <h5 class="modal-title">
                    <i class="fas fa-edit me-2"></i> Edit Barangay Admin
                </h5>
                <button type="button" class="btn-close btn-close-white" data-bs-dismiss="modal" aria-label="Close"></button>
            </div>
            <div class="modal-body">
            ...
            <div class="modal-footer">
                ...
                <button type="button" class="btn btn-black" id="confirmEditBtn">
    ...
    <div class="modal-dialog modal-dialog-centered">
        <form method="POST" class="modal-content">
            ...
            <div class="modal-body text-center py-3">
                ...
                <h5 class="mb-2">Are you sure you want to remove this admin?</h5>
            ...
            <div class="modal-footer">
                ...
                <button type="submit" name="delete_admin" class="btn btn-black">
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    document.getElementById('addAdminForm').addEventListener('submit', function() {
        document.querySelector('.loading-overlay').style.display = 'flex';
    });
    ...
    // Prevent modal hiding behind header by adjusting z-index
